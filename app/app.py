...
def index():
    user = {'username': 'Male Oscar Winner Ages'}
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM MaleOscarAges ORDER BY fldyear')
...
def form_update_post(age_id):
    ...
    sql_update_query = """UPDATE MaleOscarAges t SET t.fldYear = %s, t.fldAge = %s, t.fldName = %s, t.fldFilm = 
    %s WHERE t.id = %s """
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO MaleOscarAges (fldYear,fldAge,fldName,fldFilm) VALUES (%s,%s,%s,%s) """
...
def form_delete_post(age_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM MaleOscarAges WHERE id = %s """
...
def api_browse() -> str:
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM MaleOscarAges')
    result = cursor.fetchall()
