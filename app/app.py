...
def index():
    ...
    cursor.execute('SELECT * FROM tbladdressesImport')
...
def record_view(city_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM tbladdressesImport WHERE id=%s', city_id)
    result = cursor.fetchall()
...
def form_update_post(city_id):
    ...
    sql_update_query = """UPDATE tbladdressesImport t SET t.fldName = %s, t.fldLat = %s, t.fldLong = %s, t.fldCountry = 
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO tbladdressesImport (fldName,fldLat,fldLong,fldCountry,fldAbbreviation,fldCapitalStatus,fldPopulation) VALUES (%s, %s,%s, %s,%s, %s,%s) """
...
def form_delete_post(city_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM tbladdressesImport WHERE id = %s """
